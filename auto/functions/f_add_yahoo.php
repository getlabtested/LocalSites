...
<table width="100%">
	<tr>
		...
		<center>		
				...
				<div style="font-size:14px;width:500;text-align:left;margin-left:auto;margin-right:auto;font-weight:600;">Add Yahoo Answers Campaign 
				</div>
				...
				<table width=500 style="margin-left:auto;margin-right:auto;border: solid 1px #eeeeee;"> 
					...
					<tr>
						<td  align=left valign=top style="font-size:13px;">
							<img src="./../nav/tip.png" style="cursor:pointer;" border=0 title="Search Query Here. Seperate multiple queries with commas and on each run a unique query will be selected. The phrase 'Run' constitues a solo-run or a cronjob run, cycling through all available items.">
							Search Query<br> </td>
						<td align=right style="font-size:13px;">
							<input name="query" id="id_query" size=30 value="<?php IF ($campaign_query){ echo $campaign_query; }?>">
						</td>
					</tr>
					<tr>
						<td  align=left valign=top style="font-size:13px;">
				  ...
				  <tr>
					 ...
					 <td align=right style="font-size:13px;">
						<select id=articles_selects_languages name="language" tabindex="0">					
							...
							<option valuept-PT <?php if ($campaign_language=='pt-PT'){ echo "selected=true"; } ?>>Portuguese</option>
							...
							<select name=author>
								...
								if ($campaign_author)
								{
									foreach ($authors_id as $k=>$v)
									{
										...
										{
											echo "<option value='rand' selected=true>Random Author</option>";
										}
										else
										{
