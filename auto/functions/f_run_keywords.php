...
foreach ($campaign_id as $key=>$val)
{
	...
	if ($this_mode=='cron_config'&&$campaign_type[$key]=='keywords'&&$campaign_status[$key]=='1'||$this_mode=='solo'&&$campaign_id[$key]==$cid)
	{	
		...
		$keywords = explode(';',$campaign_query[$key]);
		
		if ($campaign_source[$key]=='wptraffictools')
		{
			$keywords = $wptt_keywords;
		}
		...
		{
			//echo $v;exit;
			$this_title_template  = $campaign_title_template[$key];
			$this_title_template = str_replace("%keyword%",$v,$this_title_template);
			$this_post_template  = 	$campaign_post_template[$key];
			$this_post_template = str_replace("%keyword%",$v,$this_post_template);
			
			$description_content = hook_content($this_post_template,$table_prefix, $store_images_relative_path,$store_images_full_url,$campaign_name[$key],$campaign_query[$key],$title,$link,$description,$templates_custom_variable_token,$templates_custom_variable_content);	
			$description = $description_content;

			//populate title template
			$title_content = hook_content($this_title_template,$table_prefix, $store_images_relative_path,$store_images_full_url,$campaign_name[$key],$campaign_query[$key],$title,$link,$description,$templates_custom_variable_token,$templates_custom_variable_content);	
			$title= $title_content;	
			...
			//procede if original				
			if ($row_count_1==0&&$row_count_2==0&&$title&&$stop==0&&!strstr($link, $blog_url)&&$campaign_post_overwrite[$key]!=1)
			{	
				...
				{ 
					...
					if (strlen($description)>25) 
					{
						...
						if ($cat!='x')
						{
							...
							{
								$query ="SELECT ID FROM {$table_prefix}posts WHERE original_source='$title";
								$result = mysql_query($query);
							...
							$query = "UPDATE ".$table_prefix."posts SET original_source='$link', bs_campaign_id='$title' WHERE ID='$post_id'";
							$result = mysql_query($query);					
