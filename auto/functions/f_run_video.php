...
foreach ($campaign_id as $key=>$val)
{
	...
	if ($this_mode=='cron_config'&&$campaign_type[$key]=='video'&&$campaign_status[$key]=='1'||$this_mode=='solo'&&$campaign_id[$key]==$cid)
	{
		...
		foreach ($links as $k=>$v)
		{
			...
			{	
				...
				if ($store_images==1) 
				{
					$thumbnail = save_image($thumbnail,$title, $k);
				}
				...
				{
					$description = strip_tags($description,'<ul><li><ol><pre><img><div><table><tr><td><i><b><p><span><u><font><tbody><h1><h2><h3><h4><br><embed><object><small><label><br/>');
				}
				if ($campaign_strip_links[$key]==2)
				{				
					$nutags= links_to_tags($description);
					//echo $tags; exit;
				...
				{
					$language="spin";
					$description = spin_text($description, $link, $title, 'youtube',$language);
				...
				$title_content = hook_content($campaign_title_template[$key],$table_prefix, $store_images_relative_path,$store_images_full_url,$campaign_name[$key],$campaign_query[$key],$title,$link,$nocontent,$templates_custom_variable_token,$templates_custom_variable_content);	
				$title= $title_content;
				...
				//declare store arrays
				$comments_names = array();
				...
				if ($campaign_language[$key]!="no translation")
				{
						...
						//echo  $translate_comments; exit;
						$translate_comments = spin_text($translate_comments , $link, $title, 'youtube',$campaign_language[$key]);
				...
				if ($campaign_spin_text[$key]==1||$campaign_spin_text[$key]==3)
				{
					$language="spin";
				...
				//if spin text is on then spin comments		
				if ($campaign_spin_text[$key]==5||$campaign_spin_text[$key]==6)
				{
					$language="salt";
				...
				//place comments in inputs for travel
				$i_comments_names = addslashes(implode("***n***",$comments_names));
				$i_comments_content = addslashes(htmlspecialchars(implode("***c***", $comments_content)));
							...
							if ($campaign_custom_field_name[$key])
							{
								...
								foreach ($campaign_custom_field_value[$key] as $k=>$v)
								{
									...
									else if ($v=='%video_embed%')
									{
										...
										$object = str_replace('%video_embed%', $object, $v);
										add_post_meta($post_id, $campaign_custom_field_name[$key][$k], $object, true);
										
									}
									else if ($v=='%video_description%')
									{
										$object = str_replace('%video_description%', $lite_description, $v);
										add_post_meta($post_id, $campaign_custom_field_name[$key][$k], $object, true);
										
									}
									else if ($v=='%video_thumbnail%')
									{
										$object = str_replace('%video_thumbnail%', $thumbnail, $v);
										add_post_meta($post_id, $campaign_custom_field_name[$key][$k], $object, true);
