...
foreach ($campaign_id as $key=>$val)
{
	...
	if ($this_mode=='cron_config'&&$campaign_type[$key]=='yahoo'&&$campaign_status[$key]=='1'||$this_mode=='solo'&&$campaign_id[$key]==$cid)
	{		
		...
		foreach ($links as $k=>$v)
		{
			...
			if (!$description){$stop=1;}else{$stop=0;}
					
			//procede if original				
			if ($row_count_1==0&&$row_count_2==0&&$stop!=1&&$title)
			{	
				$campaign_scrape_comments_status[$key]==1;
				...
				if (strlen($description)>10) 
				{
					...
					{		
						...
						if ($cat!='x')
						{
							...
							{
								//print_r($comments_names);
								//echo $campaign_comments_limit[$key];exit;
								if ($campaign_comments_limit[$key]!=0)
								{
									$comments_names = array_slice($comments_names, 0, $campaign_comments_limit[$key]);
									$comments_content = array_slice($comments_content, 0, $campaign_comments_limit[$key]);
								}
								//echo "****";
								//print_r($comments_names);
			...
			else
			{
				if ($row_count_2>0)
				{
					echo "Stats: BLOCKED.<hr>";
				}
				else if ($stop==1)
				{
					echo "Stats: NO CONTENT SCRAPED / IGNORE<hr>";
				}
				else
				{
					echo "Stats: ALREADY PUBLISHED.<hr>";
				}
			}
		}//foreach link
