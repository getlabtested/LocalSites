...
$default_title_template = $array[0];
$default_post_template = $array[1];
...
$(document).ready(function() 
{
	...
	{
		echo "$('.comments_include_select').val('0');";
	}
	?>
...
<table width="100%">
	<tr>
		<td width='50%' valign='top'>
			...
			<center>			   
			<div style="font-size:14px;width:500;text-align:left;margin-left:auto;margin-right:auto;font-weight:600;">Add YouTube Video Campaign
			</div>
			<hr width=500 style="color:#eeeeee;background-color:#eeeeee;">

			<table id=video_select_insert width=500 style='margin-bottom:10px;margin-left:auto;margin-right:auto;border: solid 1px #eeeeee'>		 
				<tr>
					 ...
					 <td align=right style="font-size:13px;">			   
						 <input name=campaign_name size=37 id='id_campaign_name' value='<?php echo $campaign_name; ?>'>
					 </td>
				</tr>
				<tr>
					 <td  align=left valign=top style='font-size:13px;'>
						<img src='./../nav/tip.png' style='cursor:pointer;' border=0 title='Example search phrase: keyword1 keyword2 -excludekeyword.'> 
						Type of Search:
					  </td>
					  <td align=right style='font-size:13px;'>
							<select id='id_select_search_type' name='source'>
								<option value='query' <?php if ($campaign_source=='query'){echo "selected='true'";} ?>>By Search Query</option>
								<option value='username' <?php if ($campaign_source=='username'){echo "selected='true'";} ?>>By User</option>
							</select>
					  </td>
				</tr>
				<tr>
					 <td  id='id_td_source_label' align=left valign=top style='font-size:13px;'>
						<img src='./../nav/tip.png' style='cursor:pointer;' border=0 title="Example search phrase: keyword1 keyword2 -excludekeyword. Seperate multiple queries with commas and on each run a unique query will be selected. The phrase 'Run' constitues a solo-run or a cronjob run, cycling through all available items. "> 
						Query:
					  </td>
					  <td align=right style='font-size:13px;'>
						<input  name=query size=30 id='id_query' value='<?php echo $campaign_query; ?>'>
					  </td>
				</tr>
