    ...
    class TableEditor
    {
        ...
        */
        function TableEditor($db, $table)
        {
            ...
            */
            ...
            $this->config['perPage']        = 100;
            $this->config['allowPKEditing'] = false;
            $this->config['allowView']      = false;
            $this->config['allowCSV']       = false;
            $this->config['allowAdd']       = false;
        ...
        */
        function display()
        {
            ...
            */
            $nospecialchars = $this->getConfig('nospecialchars');
            foreach ($this->fields as $field => $v) {
                if (empty($v['noDisplay']) AND (!is_array($nospecialchars) OR !in_array($field, $nospecialchars)) ) {
...
<table border="0" align="center">
    ...
    <tr>
		<td>
            <script>
                /**
                * This function provides select all functionality
                */
                ...
                function ToggleSelectAll ()
                {
                    ...
                    for (var i=0; i<els.length; ++i) {
                        if (els[i].type && els[i].type == 'checkbox') {
                            ...
                            els[i].parentNode.parentNode.className = checked_status ? 'highlightedRow' : '';
                        }
                    }
                }
            </script>
            ...
            <a onclick="ToggleSelectAll()">
                <img src="../nav/dot.png" width="20" height="20" alt="Select all/none" border="0" style="cursor: hand; cursor: pointer" />
            </a>
        </td>
        ...
        <?foreach($results as $k => $row):?>
            <tr <?=($k % 2 == 1 ? 'class="altRow"' : '')?> style="cursor: default" onclick="row_highlight(event, this, this.getElementsByTagName('input')[0], '<?=($k % 2 == 1 ? 'altRow' : '')?>')">
                <td>
                    <input type="checkbox"
                           id="rowSelector"
                           value="<?=$nonFilteredData[$k][$this->pk] ?>"
                ...
                <?foreach($row as $field => $v):?>
                    <?if(empty($this->fields[$field]['noDisplay'])):?>
						...
						?>
                        <td align="<?php echo $align; ?>" valign="top"><?=$v // Do not do htmlspecialchars here - it's now implemented as a display filter?></td>
                    <?endif?>
                <?endforeach?>
            </tr>
        <?endforeach?>
...
<?php
        ...
        */
        function displayActionButtons()
        {
			...
			echo '<button onclick="checkIndex()" class="actionButton" style=""width:100px;">Check Index Status</button>';
            if ($this->getConfig('allowDelete')) echo '<button onclick="row_delete()" class="actionButton">Delete From Queue</button>';
        }
    ...
    <!--
        ...
        */
        function checkIndex()
        {
            ...
            if (selectedRows.length == 0) {
                alert('You must select one or more rows!');
                return;
            }

            for (var i=0; i<selectedRows.length; ++i) {
                indexes[indexes.length] = 'indexes[]=' + encodeURIComponent(selectedRows[i]);
            }

            location.href = '../includes/tableeditor/discover_index.php?' + (location.search.length ? '&' : '&') + indexes.join('&');
...
</headiew<body onload="onload_handler()">

<div class="headerBar">
    BlogSense Bookmarking Queue
</div>
