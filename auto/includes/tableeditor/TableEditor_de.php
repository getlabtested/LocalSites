        ...
        */
        function display()
        {
            ...
            */
            if (isset($_GET['edit'])) {
                ...
                if ($this->getConfig('allowCopy')) {
                    ...
                    $this->errors[] = 'Kopieren von Zeilen ist nicht erlaubt';
                }
                ...
                if ($this->getConfig('allowAdd')) {
                    ...
                    $this->errors[] = 'Hinzufügen von Zeilen ist nicht erlaubt';
                }
            }
            ...
            */
            if (isset($_GET['view'])) {
                if ($this->getConfig('allowView')) {
                    ...
                    $this->errors[] = 'Detailansicht von Zeilen ist nicht erlaubt';
                }
            }
            ...
            */
            if (isset($_GET['delete'])) {
                if ($this->getConfig('allowDelete')) {
                    ...
                    $this->errors[] = 'Fehler beim Löschen der Zeile!';

                } else {
                    $this->errors[] = 'Löschen von Zeilen ist nicht erlaubt';
                }
            }
            ...
            */
            ...
            if ($results === false) {
                $this->errors[] = 'Fehler bei der Abfrage der Daten. Datenbank-Fehler: ' . $this->dbError();
            }
            ...
            */
            if (!empty($_GET['csvdownload'])) {
                if ($this->getConfig('allowCSV')) {
                    ...
                    $this->addError('CSV-Download ist nicht erlaubt');
                }
            }
...
<table border="0" align="center">
    <tr>
        ...
        <td colspan="<?=count($this->fields)?>" align="center">
            Zeige [<?=($total > 0 ? $startOffset + 1 : 0)?> - <?=min($startOffset + $perPage, $total)?>] von <?=$total?> Zeilen
        </td>
    </tr>
        ...
        <td colspan="<?=count($this->fields) + 1?>" align="center">
            ...
            <?if($this->getConfig('searchableFields')):?>
                <div style="float: left; text-align: left">
                    ...
                    <button onclick="search()">Suche &raquo;</button>&nbsp;<br>
                    
                    <?if($this->getConfig('allowASearch')):?>
                        <?if(!empty($_GET['asearch'])):?>
                            <i>Erweiterte Suche: <a href="<?=$modifyURL?>">Bearbeiten</a> :: <a href="<?=$clearURL?>">Zurücksetzen</a></i>
                        <?else:?>
                            <a href="<?=$aSearchURL?>"><i>Erweiterte Suche</i></a>
                        <?endif?>
                    ...
                    <?if(isset($this->search)):?>
                        <a href="<?=$clearURL?>"><i>Zurücksetzen</i></a>
                    <?endif?>
                </div>
            <?endif?>
    ...
    <?else:?>
        ...
        <tr>
            <td colspan="<?=count($this->fields)?>" align="center">Keine passenden Zeilen gefunden!</td>
        </tr>
    <?endif?>
...
<?php
        ...
        */
        ...
        {
            if ($this->getConfig('allowView'))   echo '<button onclick="row_view()" class="actionButton" style="width: 90px" >Detailansicht</button>&nbsp;';
            if ($this->getConfig('allowCSV'))    echo '<button onclick="event.cancelBubble = true; Fade(document.getElementById(\'csvDownloadLayer\'), true); document.body.scrollTop = 0" class="actionButton" style="width: 50px" id="actionCSV">CSV</button>&nbsp;';
            if ($this->getConfig('allowAdd'))    echo '<button onclick="row_add()" class="actionButton">Hinzuf&uuml;gen</button>&nbsp;';
            if ($this->getConfig('allowEdit'))   echo '<button onclick="row_edit()" class="actionButton">Bearbeiten</button>&nbsp;';
            if ($this->getConfig('allowCopy'))   echo '<button onclick="row_copy()" class="actionButton">Kopieren</button>&nbsp;';
            if ($this->getConfig('allowDelete')) echo '<button onclick="row_delete()" class="actionButton">L&ouml;schen</button>';
        }
        ...
        */
        function displayAdvancedSearchPage()
        {
            ...
            $operators[] = array('value' => '%',  'text' => 'Enthält');
            $operators[] = array('value' => '=',  'text' => 'Exakt gleich');
            $operators[] = array('value' => '>',  'text' => 'Ist größer');
            $operators[] = array('value' => '>=', 'text' => 'Ist größer oder gleich');
            $operators[] = array('value' => '<',  'text' => 'Ist kleiner');
            $operators[] = array('value' => '<=', 'text' => 'Ist kleiner oder gleich');
        ...
        */
        ...
        for (i=0; i<fields.length; ++i) {
            ...
            if (f_val) {
                ...
                if (o_val == '%' && v_val == '') {
                    alert('Bei der Verwendung des "Enthält"-Vergleichsoperator muss ein gesuchter Text angegeben werden');
                    return;
                }
        ...
        if (parameters.length == 0) {
            alert('Bitte geben Sie Suchkriterien an!');
            return;
        }
...
<table border="0" id="searchCriteriaTable">
        ...
        <tr <?if($i == count($criteria) - 1):?>id="searchCriteriaRow"<?endif?> >
            <td>
                <?if($i == count($criteria) - 1):?>
                    <button onclick="addCriteria()">Kriterium hinzufügen &raquo;</button>
                <?endif?>
            </td>
    ...
    <tr id="searchCriteriaTable_lastRow">
        ...
        <td>
            <input type="radio" name="match" value="any" id="match_any" <?=(!isset($_GET['match']) || $_GET['match'] == 'any' ? 'checked' : '')?> > <label for="match_any">die ein Kriterium erfüllen</label><br>
            <input type="radio" name="match" value="all" id="match_all" <?=(isset($_GET['match']) && $_GET['match'] == 'all' ? 'checked' : '')?> > <label for="match_all">die alle Kriterien erfüllen</label>
        </td>
        
        <td valign="top" align="right">
            <button onclick="location.href = '<?=$cancelURL?>'"><?=$this->label_cancel?></button>&nbsp;
            <button onclick="search()">Suchen &raquo;</button>
        </td>
    </tr>
</table>
            ...
            if (is_array($_GET['fields'])) {
                ...
                foreach ($_GET['fields'] as $i => $f) {
                    if (!in_array($f, $this->getConfig('searchableFields'))) {
                        $this->errors[] = 'Ungültiges Feld im Suchkriterium enthalten';
                        continue;
                    }
        ...
        */
        function handleView($id)
        {
            ...
            */
            ...
            if ($row === false) {
                $this->errors[] = 'Die angegebene Zeile konnte nicht gefunden werden';
                return;
            }
...
<table>
    ...
    <tr>
        <td colspan="2" align="right">
            <button class="actionButton" onclick="location.href = '<?=$okURL?>'"><?=$this->label_ok?></button>&nbsp;
            
            <?if($this->getConfig('allowEdit')):?>
                <button class="actionButton" onclick="location.href = '<?=$editURL?>'">Bearbeiten</button>
            <?endif?>
        </td>
    </tr>
</table>
        ...
        */
        function handleAddEditCopy($id = null)
        {
            ...
            if (!empty($_POST)) {
                ...
                // Cancel button
                if ($_POST['action'] == $this->label_cancel) {
                    $url = new TableEditor_URL();
                    ...
                    switch ($this->fields[$field]['input']) {
                        case 'password':
                            if (!empty($_POST[$field . '_blank'])) {
                                ...
                                $this->setContextualError($field, 'Die angegebenen Passwörter stimmen nicht überein');
                            }
                            break;
                        ...
                        case 'email':
                            ...
                            if (!empty($_POST[$field]) AND !preg_match($regex, $_POST[$field])) {
                                $this->setContextualError($field, "Keine gültige eMail-Adresse");
                            }
                            break;
                ...
                */
                if (isset($_GET['edit'])) {
                    ...
                    */
                    ...
                    if (!$res) {
                        $this->errors[] = "Fehler beim Aktualisieren der Zeile: " . $this->dbError();
                ...
                */
                    ...
                    if (!$res) {
                        $this->errors[] = "Fehler beim Hinzufügen der Zeile: " . $this->dbError();
                    } else {
                ...
                */
                if (empty($this->errors)) {
                    ...
                    } else if ($_POST['action'] == $this->label_apply) {
                        $url = new TableEditor_URL();
        ...
        function displayAddEditCopyPage($id) // $id is already SQL ready
        {
            ...
            if (isset($_GET['edit']) OR isset($_GET['copy'])) {
                ...
                */
                ...
                $title = isset($_GET['edit']) ? 'bearbeiten' : 'kopieren';
...
<table border="0">
    ...
    <tr>
        <td colspan="3">
            <span class="requiredAsterisk">*</span> Benötigte Felder
        </td>
    </tr>

    <tr>
        <td colspan="3" align="right">
            <input type="submit" name="action" style="width: 80px" value="<?=$this->label_ok?>">
            <input type="submit" name="action" style="width: 80px" value="<?=$this->label_cancel?>">
            <input type="submit" name="action" style="width: 80px" value="<?=$this->label_apply?>" disabled>
        </td>
    </tr>
</table>
    ...
    <!--
            ...
            switch (selectObj.options[selectObj.selectedIndex].value) {
                ...
                case 'selected':
                    ...
                    if (selectedRows.length == 0) {
                        alert('Bitte wählen Sie Zeilen zum Download aus!');
                        return;
                    }
        ...
        */
        function row_view()
        {
            ...
            if (selectedRows.length == 0) {
                alert('Für die Detailansicht muss eine Zeile ausgewählt sein!');
                return;
            }

            if (selectedRows.length > 1) {
                alert('Es darf nur genau eine Zeile für die Detailansicht ausgewählt sein');
                return;
        ...
        */
        function row_delete()
        {
            ...
            if (selectedRows.length == 0) {
                alert('Sie müssen eine oder mehrere Zeilen zum Löschen auswählen!');
                return;
            }
            ...
            if (confirm('Sind Sie sicher dass die ausgewählten Zeilen gelöscht werden sollen? ' + (deletes.length > 1 ? '\nACHTUNG: Mehrere Zeilen sind ausgewählt!' : ''))) {
        ...
        */
        function row_edit()
        {
            ...
            if (selectedRows.length == 0) {
                alert('Zum Bearbeiten muss eine Zeile ausgewählt sein!');
                return;
            }

            if (selectedRows.length > 1) {
                alert('Es darf nur genau eine Zeile zum Bearbeiten auswählt sein');
                return;
        ...
        */
        function row_copy()
        {
            ...
            if (selectedRows.length == 0) {
                alert('Zum Kopieren muss eine Zeile ausgewählt sein!');
                return;
            }

            if (selectedRows.length > 1) {
                alert('Es darf nur genau eine Zeile zum Kopieren auswählt sein');
                return;
...
<div class="headerBar">
    ...
    <a href="http://www.phpguru.org" target="_blank" style="text-decoration: none; font-size: 10pt">MySQL TableEditor von Richard Heyes</a>
</div>
            ...
            <tr>
                ...
                <td>
                    <select id="csvdownload_what" align="absmiddle">
                        <option value="">Auswählen...</option>
                        <option value="selected">Ausgewählte Zeilen</option>
                        <option value="page">Diese Seite</option>
                        <option value="table">Gesamte Tabelle</option>
                    </select>
                </td>
            </tr>
        
            <tr>
                <td><label for="include_headers">Kopfzeilen einfügen?</label></td>
